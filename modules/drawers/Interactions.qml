    ...
    onPositionChanged: event => {
        ...
        if (Config.bar.showOnHover) {
            if (x < bar.clampedWidth) {
                if (!screenState.bar && !bar.isHovered && !hoverTimer.showBar) {
        ...
        if (panels.sidebar.offsetScale === 1) {
            ...
            if (showOsd) {
                if (!screenState.osd && !hoverTimer.showOsd) {
                    hoverTimer.queue("osd");
                } else if (screenState.osd && osdShortcutActive) {
        ...
        if (Config.launcher.showOnHover) {
            if (inBottomPanel(panels.launcher, x, y)) {
                if (!screenState.launcher && !hoverTimer.showLauncher) {
        ...
        if (showDashboard) {
            if (!screenState.dashboard && !hoverTimer.showDashboard) {
                hoverTimer.queue("dashboard");
            } else if (screenState.dashboard && dashboardShortcutActive) {
        ...
        if (showUtilities) {
            if (!screenState.utilities && !hoverTimer.showUtilities) {
                hoverTimer.queue("utilities");
            } else if (screenState.utilities && utilitiesShortcutActive) {
